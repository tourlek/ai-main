thread_id: 019f654e-423f-7483-bdd6-494aba0e6b12
...
# Cross-repo review of MR !1285 for unread/unresponded found core improvements but still had deploy/correctness blockers.

Rollout context: The user asked for a thorough, findings-first review of MR !1285 (unread/unresponded only) with exact file:line evidence, and wanted prior review context consulted first without re-flagging already-fixed items. The review used the `code-review` skill and compared MR head `56351ecd07ac32558ef258f5a8603a26a54e4f43` against base `33fea2c5042031f826ca5369c88438b9e0f30cd9` from the exact MR, not the dirty working tree.
...
Preference signals:
- The user asked to read prior review docs first and “do not re-flag findings already documented as fixed there” -> future reviews should rebase on prior review history and avoid duplicate findings.
- The user asked for “structured findings report, ranked by severity” and “every finding must cite an exact file:line” -> future similar reviews should stay line-precise, severity-ranked, and evidence-first.
- The user said “do not modify any files” -> review flows should stay read-only.
...
- The contact-side scoped broadcast emitter is correct for the unresponded flag, but it scopes to channel-eligible members only; it does not account for sale-visibility audience restrictions used by contact search. Treat that as an audience mismatch, not a flag-gate bug.
...
Preference signals:
- The user wanted the review to “cover all 3 repos” and to separate findings by repo/axis -> future similar reviews should keep repo boundaries and audit tables explicit.
...
- Validation was limited by the sandbox: `git diff --check` and `node --check` passed, but targeted Jest could not run because the read-only environment blocked Jest temp writes and surfaced duplicate mock collisions.
